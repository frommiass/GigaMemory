#!/usr/bin/env python3
"""
Тест RAG компонентов с прямым импортом
"""
import sys
import os
...
def test_individual_components():
    ...
    # Тест 7: RAG движок (без LLM)
    print("\n7️⃣ Тестирование RAG движка:")
    try:
        from submit.rag.engine import RAGEngine
        ...
        rag = RAGEngine()
        prompt, metadata = rag.process_question("Какие у меня животные?", "test_dialogue", test_messages)
        
        print(f"   Стратегия: {metadata.get('strategy', 'N/A')}")
        print(f"   Тема: {metadata.get('topic', 'N/A')}")
        print(f"   Выбрано сессий: {metadata.get('selected_sessions', 0)}")
        print(f"   Длина промпта: {len(prompt)}")
        
        print("   ✅ RAG движок работает!")
    except Exception as e:
        print(f"   ❌ Ошибка в RAG движке: {e}")
