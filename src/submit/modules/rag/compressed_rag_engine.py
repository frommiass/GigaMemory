...
# from .vector_rag_engine import VectorRAGEngine, VectorRAGConfig  # Убираем несуществующий модуль
from ..compression.hierarchical_compressor import HierarchicalCompressor
from ..compression.compression_models import (
    CompressionConfig, 
    CompressionLevel,
    CompressionMethod,
    CompressionResult
)
# from .prompts.topic_prompts import get_topic_prompt  # Убираем несуществующий модуль
# from .prompts.fallback_prompts import get_fallback_prompt  # Убираем несуществующий модуль
...
class CompressedRAGEngine:  # Убираем наследование от VectorRAGEngine
    ...
    def __init__(self, config: Optional[CompressedRAGConfig] = None):
        ...
        # super().__init__(self.config)  # Убираем из-за отсутствующего родительского класса
    ...
    def _compress_session(self, session_id: str, messages: List[Message]) -> str:
        ...
        from ..storage.filters.session_grouper import extract_session_content
    ...
    def _process_with_compressed_topic_rag(self, question: str, topic: str, confidence: float,
        ...
        # prompt = get_topic_prompt(topic, question, memory_text)  # Убираем из-за отсутствующего модуля
        prompt = f"Тема: {topic}\nВопрос: {question}\nКонтекст: {memory_text}"
    ...
    def _process_with_compressed_fallback(self, question: str, dialogue_id: str) -> Tuple[str, Dict]:
        ...
        # prompt = get_fallback_prompt(question, memory_text)  # Убираем из-за отсутствующего модуля
