#!/usr/bin/env python
"""
Финальная правильная версия скрипта с улучшенной фильтрацией
"""
import argparse
import json
import sys
...
def is_copy_paste_content(content: str) -> bool:
    ...
    # Длинные сообщения (>200 символов) почти всегда копипаст
    if len(content) > 200:
        return True
...
def extract_user_messages_only(messages: List[Dict[str, Any]]) -> List[str]:
    """
    Извлекает ТОЛЬКО сообщения пользователя с улучшенной фильтрацией
    """
    user_messages = []
    ...
    for msg in messages:
        ...
        # Проверяем длину - более мягкие ограничения
...
def extract_facts_by_question_topic(user_messages: List[str], question: str) -> List[str]:
    ...
    if 'спорт' in question_lower or 'занимаюсь' in question_lower:
        ...
        for message in user_messages:
            ...
            if 'спорт' in message_lower:
                ...
                elif 'пинг-понг' in message_lower:
                    facts.append("Играет в пинг-понг")
        ...
        for message in user_messages:
            ...
            if 'работаю' in message_lower:
                if 'яндексе' in message_lower:
                    ...
                    facts.append("Работает учителем")
                elif 'врач' in message_lower:
                    facts.append("Работает врачом")
    ...
    elif 'сигареты' in question_lower or 'курю' in question_lower:
        # Ищем информацию о сигаретах
        for message in user_messages:
            message_lower = message.lower()
            if 'сигарет' in message_lower or 'курю' in message_lower or 'курить' in message_lower:
                if 'марка' in message_lower:
                    # Ищем марку сигарет
                    brand_match = re.search(r'марка\s+([а-яё\w]+)', message_lower)
                    if brand_match:
                        facts.append(f"Предпочитает сигареты марки {brand_match.group(1)}")
                elif 'пачка' in message_lower:
                    facts.append("Курит сигареты")
...
def create_prompt_from_dialogue(dialogue: Dict[str, Any]) -> str:
    ...
    if all_user_messages:
        for i, msg in enumerate(all_user_messages[-10:], 1):  # Последние 10 сообщений
...
def run_inference(dataset_path: str, output_path: str):
    ...
    print(f"🚀 Запуск ФИНАЛЬНОГО ПРАВИЛЬНОГО инференса на датасете: {dataset_path}")
    ...
    print("✅ ФИНАЛЬНЫЙ ПРАВИЛЬНЫЙ инференс завершен успешно!")
...
def main():
    """Основная функция"""
    parser = argparse.ArgumentParser(description="Final Correct Dialogue Inference")
    parser.add_argument("--dataset", type=str, required=True, help="Путь к датасету для инференса")
    parser.add_argument("--output", type=str, default="./final_correct_output", help="Путь для сохранения результатов")
